...
const assertBillingPopupVisibleOrFailFast = async (page) => {
  try {
    await expect(page.getByRole('heading', { name: /insufficient tokens/i })).toBeVisible({
      timeout: 10_000,
    })
...
test('ENCOUNTERS-VIEW-RESET-HISTORY-01 resets conversation history from influence panel', async ({
  page,
}) => {
  await page.goto('/encounters')
  await waitForEncountersGet(page)
  ...
  const resetButton = page.locator('.reset-history-button')
  await expect(resetButton).toBeVisible()
  await expect(resetButton).toHaveText('🔄')

  const playerName = (await page.locator('#player-select option:checked').innerText()).trim()
  const characterName = (await page.locator('.character-name-link').first().innerText()).trim()
  ...
  const refreshedConversationResponsePromise = page.waitForResponse((response) => {
    return (
      /\/api\/encounters\/\d+\/conversation\/\d+\/\d+$/.test(response.url()) &&
      response.request().method() === 'GET'
    )
  ...
  await clickWhenActionable(resetButton)

  const deleteResponse = await deleteResponsePromise
  expect(deleteResponse.status()).toBe(204)

  const refreshedConversationResponse = await refreshedConversationResponsePromise
  ...
  await clickWhenActionable(challengeButton)
  await playerSelect.selectOption('')
  await expect(page.locator('.reset-history-button')).toHaveCount(0)
})
...
test('ENCOUNTERS-TALKING-AUDIO-01 returns audio over websocket and completes processing @audio', async ({
  ...
  try {
    await page.waitForSelector('.encounter-popup audio', { state: 'attached' })
    await page.evaluate(() => {
      const audio = document.querySelector('.encounter-popup audio')
      if (audio) {
    ...
    await expect(page.locator('.shared-speak-button')).toBeDisabled()
    await expect(page.locator('.shared-speak-button')).toBeEnabled({ timeout: 60_000 })
