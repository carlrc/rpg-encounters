<template>
  <div class="bias-preference-row">
    ...
    <div v-if="selectedOption && !selectedOption.startsWith('__temp_')" class="bias-slider-container">
      <input 
        type="range" 
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const selectedOption = ref(props.initialOption && props.initialOption.startsWith('__temp_') ? '' : props.initialOption)
    const sliderValue = ref(props.initialValue)

    const availableOptions = computed(() => {
      return props.options.filter(option => 
        !props.usedOptions.includes(option) || option === selectedOption.value
      )
    })
    ...
    watch(() => props.initialOption, (newVal) => {
