...
export function useFileImport(entityType) {
    ...
    const parseEntitySection = (section) => {
        ...
        if (!entity.name || !entity.appearance || !entity.race || !entity.class_name || !entity.size || !entity.alignment) {
            throw new Error(`Missing required fields for ${entityType.toLowerCase()}: ${entity.name || 'Unknown'}`)
        }
    ...
    const handleImportFile = async (event, createEntityFn, onSuccess, onError) => {
        ...
        try {
            ...
            const { successCount, errors } = await importEntitiesFromMarkdown(parsedEntities, createEntityFn)
            ...
            if (successCount > 0) {
                ...
                if (errors.length > 0) {
                    onError(`${message}. However, ${errors.length} ${entityType.toLowerCase()}${errors.length > 1 ? 's' : ''} failed to import:\n\n${errors.join('\n\n')}`)
                } else {
                    onSuccess(message)
                }
            } else {
                onError(`Import failed - no ${entityType.toLowerCase()}s were successfully imported:\n\n${errors.join('\n\n')}`)
            }
