...
async def delete_conversation_history(
    ...
    """Delete conversation history and influence for a player/character in an encounter"""
    user_id, world_id = session.user_id, session.world_id
    ...
    try:
        ...
        await InfluenceStore(
            user_id=user_id, world_id=world_id, session=db_session
        ).delete(character_id=character_id, player_id=player_id)
    except HTTPException:
        raise
    except Exception as e:
        logger.error(
            "Failed to delete conversation history for player %s, character %s, encounter %s, user %s, world %s: %s",
            player_id,
            character_id,
            encounter_id,
            user_id,
