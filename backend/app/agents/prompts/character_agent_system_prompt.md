...
**NUGGET SELECTION**: Some characters have access to multiple nuggets at the same time. You must select the one that matches the conversation most closely. Only switch if the player changes the conversation.

## Response Structure

Provide THREE responses: PUBLIC (basic info), PRIVILEGED (bias-favored info), EXCLUSIVE (earned info through dialogue).
