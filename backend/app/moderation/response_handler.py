...
async def handle_moderation_response(
    ...
    try:
        record = await ModerationStore(user_id=user_id).create(
    ...
    telem_client.update_current_trace(input=f"MODERATED MESSAGE. Record: {record.id}")
    ...
    await stream_tts_audio(
        ...
        tts_provider=create_tts_provider(
            # Use the same TTS provider as normal conversation flow.
