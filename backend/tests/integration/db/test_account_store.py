...
async def test_account_store():
    ...
    async with get_async_db_session(url) as session:
        ...
        account_store = AccountStore(session=session)
        ...
        created_account = await account_store.create_account(new_account_data)
        ...
        # Test get by id
        retrieved_account = await account_store.get_account_by_id(created_account.id)
        ...
        all_accounts = await account_store.get_all_accounts()
        ...
        updated_account = await account_store.update_account(
        ...
        # Test account exists
        exists = await account_store.account_exists(created_account.id)
        assert exists is True

        # Test delete account
        deleted = await account_store.delete_account(created_account.id)
        assert deleted is True

        # Test account no longer exists
        exists_after_delete = await account_store.account_exists(created_account.id)
